...
template <typename T>
void BusinessConfig::AddInsertParam(
...
template <typename T>
void BusinessConfig::AddSetParam(
    const std::string &field, const T &value,
...
bool BusinessConfig::Create(
    const omnisphere::dtos::CreateBusinessConfig &_businessConfig) const {
  try {
    ...
    AddInsertParam("FacebookProfile", _businessConfig.FacebookProfile,
                   insertClauses, params);
    AddInsertParam("InstagramProfile", _businessConfig.InstagramProfile,
                   insertClauses, params);
    ...
    AddInsertParam("AttachmentsPath", _businessConfig.AttachmentsPath,
                   insertClauses, params);
    AddInsertParam("ImagePath", _businessConfig.ImagePath,
                   insertClauses, params);
    ...
    if (!Database->RunPrepared(sQuery, params, "BusinessConfig::Create"))
    ...
    throw std::runtime_error(std::string("[CreateBusinessConfig Exception] ") +
                             e.what());
  }
}

bool BusinessConfig::Update(
    const omnisphere::dtos::UpdateBusinessConfig &_businessConfig) const {
  try {
    ...
    AddSetParam("FacebookProfile", _businessConfig.FacebookProfile, setClauses,
                params);
    AddSetParam("InstagramProfile", _businessConfig.InstagramProfile,
                setClauses, params);
    ...
    AddSetParam("AttachmentsPath", _businessConfig.AttachmentsPath, setClauses,
                params);
    AddSetParam("ImagePath", _businessConfig.ImagePath, setClauses,
                params);
    ...
    if (!Database->RunPrepared(sQuery, params, "BusinessConfig::Update"))
      throw std::runtime_error("[RunPrepared exception]");
    ...
    Database->RollbackTransaction();
    throw std::runtime_error(std::string("[UpdateBusinessConfig Exception] ") +
                             e.what());
  }
}

omnisphere::types::DataTable BusinessConfig::Read(
    const omnisphere::dtos::GetBusinessConfig &getBusinessConfig) const {
  try {
    ...
    return Database->FetchResults(sQuery, "BusinessConfig::Read");
  } catch (const std::exception &e) {
    throw std::runtime_error(std::string("[ReadBusinessConfig Exception] ") +
                             e.what());
  }
}
} // namespace omnisphere::repositories