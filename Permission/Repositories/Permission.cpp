...
omnisphere::types::DataTable PermissionRepository::ReadAll() const {
  try {
    ...
    throw(std::runtime_error(std::string("[ReadAllPermission Exception] ") +
                             e.what()));
  }
}

omnisphere::types::DataTable PermissionRepository::Read(
    const omnisphere::dtos::GetPermission &getPermission) const {
  try {
    ...
    if (getPermission.Entry.has_value()) {
      ...
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getPermission.Entry.value()));
    }

    if (getPermission.Code.has_value()) {
      query += " AND Code = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getPermission.Code.value()));
    }

    if (getPermission.ModuleEntry.has_value()) {
      query += " AND ModuleEntry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getPermission.ModuleEntry.value()));
    }

    return database->FetchPrepared(query, parameters,
                                   "PermissionRepository::Read");
  } catch (const std::exception &e) {
    throw(std::runtime_error(std::string("[ReadPermission Exception] ") +
                             e.what()));
  }
}
} // namespace omnisphere::repositories
