...
std::vector<omnisphere::models::Permission> Permission::GetAll() const {
    try {
        ...
        return list;
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetAllPermissions Exception] ") + e.what()); }
}
...
omnisphere::models::Permission Permission::Get(const omnisphere::dtos::GetPermission& getPermission) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetPermission Exception] ") + e.what()); }
}
} // namespace omnisphere::permission
