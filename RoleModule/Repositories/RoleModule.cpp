...
bool RoleModuleRepository::Create(
    const omnisphere::dtos::CreateRoleModule &roleModule) const {
  try {
    const std::string query =
        "INSERT INTO RoleModules (Entry, RoleEntry, ModuleEntry, IsActive, "
        "CreatedBy, CreateDate) VALUES (?, ?, ?, ?, ?, GETDATE())";
    ...
    throw(std::runtime_error(std::string("[CreateRoleModule Exception] ") +
                             e.what()));
  }
}

bool RoleModuleRepository::Update(
    const omnisphere::dtos::UpdateRoleModule &roleModule) const {
  try {
    ...
    database->RollbackTransaction();
    throw(std::runtime_error(std::string("[UpdateRoleModule Exception] ") +
                             e.what()));
  }
}
...
omnisphere::types::DataTable RoleModuleRepository::ReadAll() const {
  try {
    return database->FetchResults("SELECT * FROM RoleModules",
                                  "RoleModuleRepository::ReadAll");
  } catch (const std::exception &e) {
    throw(std::runtime_error(std::string("[ReadAllRoleModule Exception] ") +
                             e.what()));
  }
}

omnisphere::types::DataTable RoleModuleRepository::Read(
    const omnisphere::dtos::GetRoleModule &getRoleModule) const {
  try {
    std::string query = "SELECT * FROM RoleModules WHERE 1=1";
    std::vector<omnisphere::types::SQLParam> parameters;

    if (getRoleModule.Entry.has_value()) {
      query += " AND Entry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getRoleModule.Entry.value()));
    }

    if (getRoleModule.RoleEntry.has_value()) {
      query += " AND RoleEntry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getRoleModule.RoleEntry.value()));
    }

    if (getRoleModule.ModuleEntry.has_value()) {
      query += " AND ModuleEntry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getRoleModule.ModuleEntry.value()));
    }

    return database->FetchPrepared(query, parameters,
                                   "RoleModuleRepository::Read");
  } catch (const std::exception &e) {
    throw(std::runtime_error(std::string("[ReadRoleModule Exception] ") +
                             e.what()));
  }
}
...
bool RoleModuleRepository::Delete(int entry) const {
  try {
    ...
    if (!database->RunPrepared("DELETE FROM RoleModules WHERE Entry = ?",
                               parameters, "RoleModuleRepository::Delete"))
      throw std::runtime_error("[RunPrepared exception]");
    ...
    database->RollbackTransaction();
    throw(std::runtime_error(std::string("[DeleteRoleModule Exception] ") +
                             e.what()));
  }
}
...
int RoleModuleRepository::GetCurrentSequence() const {
  try {
    omnisphere::types::DataTable dataTable = database->FetchResults(
        "SELECT COALESCE(RoleModSequence, 0) + 1 RoleModSequence FROM "
...
bool RoleModuleRepository::UpdateSequence() const {
  try {
    if (!database->RunStatement(
            "UPDATE Sequences SET RoleModSequence = COALESCE(RoleModSequence, "
            "0) + 1 WHERE Entry = 1"))
