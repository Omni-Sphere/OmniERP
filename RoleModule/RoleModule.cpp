...
struct RoleModule::Impl {
  ...
  explicit Impl(std::shared_ptr<omnisphere::data::Database> database)
      : repository(
            std::make_shared<omnisphere::repositories::RoleModuleRepository>(
                database)) {}
};
RoleModule::RoleModule(std::shared_ptr<omnisphere::data::Database> database)
    : pImpl(std::make_unique<Impl>(database)) {}
RoleModule::~RoleModule() = default;
omnisphere::models::RoleModule
RoleModule::Add(const omnisphere::dtos::CreateRoleModule &roleModule) const {
  if (pImpl->repository->Create(roleModule)) {
    omnisphere::dtos::GetRoleModule get;
    get.RoleEntry = roleModule.RoleEntry;
    get.ModuleEntry = roleModule.ModuleEntry;
    return Get(get);
  }
  throw std::runtime_error("Error adding role module");
}
omnisphere::models::RoleModule
RoleModule::Modify(const omnisphere::dtos::UpdateRoleModule &roleModule) const {
  if (pImpl->repository->Update(roleModule)) {
    omnisphere::dtos::GetRoleModule get;
    get.Entry = roleModule.Entry;
    return Get(get);
  }
...
std::vector<omnisphere::models::RoleModule> RoleModule::GetAll() const {
  ...
  for (int i = 0; i < data.RowsCount(); i++)
    list.push_back({data[i]["Entry"], data[i]["RoleEntry"],
                    data[i]["ModuleEntry"], data[i]["IsActive"],
...
omnisphere::models::RoleModule
RoleModule::Get(const omnisphere::dtos::GetRoleModule &getRoleModule) const {
  omnisphere::types::DataTable data = pImpl->repository->Read(getRoleModule);
  if (data.RowsCount() == 0)
    throw std::runtime_error("RoleModule doesn't exist");
  return {data[0]["Entry"],
          data[0]["RoleEntry"],
          data[0]["ModuleEntry"],
          data[0]["IsActive"],
...
bool RoleModule::Remove(int entry) const {
