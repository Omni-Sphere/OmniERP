...
struct UserModule::Impl {
  ...
  explicit Impl(std::shared_ptr<omnisphere::data::Database> database)
      : repository(
            std::make_shared<omnisphere::repositories::UserModuleRepository>(
                database)) {}
};
...
UserModule::Add(const omnisphere::dtos::CreateUserModule &userModule) const {
  try {
  ...
  {
    throw std::runtime_error(std::string("[AddUserModule Exception] ") +
                             e.what());
  }
}

omnisphere::models::UserModule
UserModule::Modify(const omnisphere::dtos::UpdateUserModule &userModule) const {
  try {
  ...
  {
    throw std::runtime_error(std::string("[ModifyUserModule Exception] ") +
                             e.what());
  }
}
...
std::vector<omnisphere::models::UserModule> UserModule::GetAll() const {
  ...
  {
    throw std::runtime_error(std::string("[GetAllUserModules Exception] ") +
                             e.what());
  }
}

omnisphere::models::UserModule
UserModule::Get(const omnisphere::dtos::GetUserModule &getUserModule) const {
  try {
    omnisphere::types::DataTable data = pImpl->repository->Read(getUserModule);

    if (data.RowsCount() == 0)
      throw std::runtime_error("UserModule doesn't exist");
    return
  ...
  {
    throw std::runtime_error(std::string("[GetUserModule Exception] ") +
                             e.what());
  }
}
...
bool UserModule::Remove(int entry) const {
  ...
  {
    throw std::runtime_error(std::string("[RemoveUserModule Exception] ") +
                             e.what());
  }
}
} // namespace omnisphere::usermodule
