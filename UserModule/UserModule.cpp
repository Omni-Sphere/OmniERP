...
omnisphere::models::UserModule UserModule::Add(const omnisphere::dtos::CreateUserModule& userModule) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[AddUserModule Exception] ") + e.what()); }
}
...
omnisphere::models::UserModule UserModule::Modify(const omnisphere::dtos::UpdateUserModule& userModule) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[ModifyUserModule Exception] ") + e.what()); }
}
...
std::vector<omnisphere::models::UserModule> UserModule::GetAll() const {
    try {
        ...
        for (int i = 0; i < data.RowsCount(); i++) {
            list.push_back({data[i]["Entry"], data[i]["UserEntry"], data[i]["ModuleEntry"],
                            data[i]["IsAllowed"],
        ...
        return list;
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetAllUserModules Exception] ") + e.what()); }
}
...
omnisphere::models::UserModule UserModule::Get(const omnisphere::dtos::GetUserModule& getUserModule) const {
    try {
        ...
        return {data[0]["Entry"], data[0]["UserEntry"], data[0]["ModuleEntry"],
                data[0]["IsAllowed"],
                data[0]["CreatedBy"], data[0]["CreateDate"],
                data[0]["LastUpdatedBy"].GetOptional<int>(),
                data[0]["UpdateDate"].GetOptional<std::string>()};
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetUserModule Exception] ") + e.what()); }
}

bool UserModule::Remove(int entry) const {
    try { return pImpl->repository->Delete(entry); }
    catch (const std::exception& e) { throw std::runtime_error(std::string("[RemoveUserModule Exception] ") + e.what()); }
}
} // namespace omnisphere::usermodule
