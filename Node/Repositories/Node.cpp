...
static const std::vector<std::string> nodeSelectFields = {
    "Entry",      "Code",          "Name",        "NodeType", "OperationMode",
    "CashLimit",  "IPAddress",     "ExtendedLog", "IsActive", "CreatedBy",
...
omnisphere::enums::OperationMode
Node::CharToOperationMode(const std::string &str) {
  if (str == "R" || str == "Restaurant")
    return omnisphere::enums::OperationMode::Restaurant;
  if (str == "T" || str == "Touch")
    return omnisphere::enums::OperationMode::Touch;
  return omnisphere::enums::OperationMode::POS;
}

omnisphere::models::Node
Node::MapNodeRow(omnisphere::types::DataTable::Row &row) {
  omnisphere::models::Node n =
      omnisphere::data::MapFromRow<omnisphere::models::Node>(row);
...
bool Node::Create(const omnisphere::dtos::CreateNode &node) const {
  try {
    ...
    static const std::vector<std::string> insertColumns = {
        "Entry",         "Code",      "Name",      "NodeType",
        "OperationMode", "CashLimit", "IPAddress", "ExtendedLog",
        "IsActive",      "CreatedBy", "CreateDate"};
    const std::string query =
        omnisphere::types::BuildInsertQuery("Nodes", insertColumns);

    std::string extVal =
        (node.ExtendedLog.has_value() && node.ExtendedLog.value()) ? "Y" : "N";
    ...
    std::vector<omnisphere::types::SQLParam> parameters = {
        ...
        omnisphere::types::MakeSQLParam(
            OperationModeToChar(node.OperationMode)),
...
bool Node::Update(const omnisphere::dtos::UpdateNode &node) const {
  try {
    ...
    const std::string query =
        omnisphere::types::BuildUpdateQuery("Nodes", setColumns, "[Entry] = ?");
...
Node::Read(const omnisphere::dtos::GetNode &filter,
           const std::vector<std::string> &fields) const {
  try {
    const std::vector<std::string> &selectFields =
        fields.empty() ? nodeSelectFields : fields;
    ...
    std::string sQuery =
        "SELECT " + qp.SelectClause + " FROM Nodes WHERE " + qp.WhereClause;
...
omnisphere::types::DataTable
Node::ReadAll(const std::vector<std::string> &fields) const {
  try {
    const std::vector<std::string> &selectFields =
        fields.empty() ? nodeSelectFields : fields;
    auto qp = omnisphere::types::BuildQueryParts(selectFields, {});
    std::string sQuery =
        "SELECT " + qp.SelectClause + " FROM Nodes WHERE IsActive = 'Y'";

    return Database->FetchResults(sQuery, "Node::ReadAll");
  } catch (const std::exception &e) {
    throw std::runtime_error(std::string("[ReadAllNode Exception] ") +
                             e.what());
  }
}
...
Node::Search(const std::vector<std::string> &fields,
  ...
  try {
    ...
    if (!filter.Name.empty()) {
      ...
      sqlParams.push_back(
          omnisphere::types::MakeSQLParam("%" + filter.Name + "%"));
    }
...
int Node::GetCurrentSequence() const {
  try {
    const std::string query = "SELECT COALESCE(NodeSequence, 0) + 1 "
                              "NodeSequence FROM Sequences WHERE Entry = 1";
    omnisphere::types::DataTable dataTable =
        Database->FetchResults(query, "Node::GetCurrentSequence");
...
bool Node::UpdateNodeSequence() const {
  try {
    const std::string query =
        "UPDATE Sequences SET NodeSequence = COALESCE(NodeSequence, 0) + 1";
    ...
    throw std::runtime_error(std::string("[UpdateNodeSequence Exception] ") +
                             e.what());
  }
}
