...
bool UserPermissionRepository::Create(
    const omnisphere::dtos::CreateUserPermission &userPermission) const {
  try {
    const std::string query =
        "INSERT INTO UserPermissions (Entry, UserEntry, PermissionEntry, "
...

  {
    ...
    throw(std::runtime_error(std::string("[CreateUserPermission Exception] ") +
                             e.what()));
  }
}

bool UserPermissionRepository::Update(
    const omnisphere::dtos::UpdateUserPermission &userPermission) const {
  try {
    ...
    if (userPermission.IsActive.has_value()) {
      ...
      parameters.push_back(omnisphere::types::MakeSQLParam(
          userPermission.IsActive.value() ? "Y" : "N"));
    }
    query += ", LastUpdatedBy = ? WHERE Entry = ?";
    parameters.push_back(
        omnisphere::types::MakeSQLParam(userPermission.UpdatedBy));
    parameters.push_back(omnisphere::types::MakeSQLParam(userPermission.Entry));

    if (!database->RunPrepared(query, parameters,
                               "UserPermissionRepository::Update"))
      throw std::runtime_error("[RunPrepared exception]");
  ...
  {
    database->RollbackTransaction();
    throw(std::runtime_error(std::string("[UpdateUserPermission Exception] ") +
                             e.what()));
  }
}
...
omnisphere::types::DataTable UserPermissionRepository::ReadAll() const {
  ...
  {
    return database->FetchResults("SELECT * FROM UserPermissions",
                                  "UserPermissionRepository::ReadAll");
  } catch (const std::exception &e)

  {
    throw(std::runtime_error(std::string("[ReadAllUserPermission Exception] ") +
                             e.what()));
  }
}

omnisphere::types::DataTable UserPermissionRepository::Read(
    const omnisphere::dtos::GetUserPermission &getUserPermission) const {
  try {
    ...
    if (getUserPermission.Entry.has_value()) {
      query += " AND Entry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getUserPermission.Entry.value()));
    }

    if (getUserPermission.UserEntry.has_value()) {
      query += " AND UserEntry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getUserPermission.UserEntry.value()));
    }

    if (getUserPermission.PermissionEntry.has_value()) {
      query += " AND PermissionEntry = ?";
      parameters.push_back(omnisphere::types::MakeSQLParam(
          getUserPermission.PermissionEntry.value()));
    }

    return database->FetchPrepared(query, parameters,
                                   "UserPermissionRepository::Read");
  } catch (const std::exception &e)

  {
    throw(std::runtime_error(std::string("[ReadUserPermission Exception] ") +
                             e.what()));
  }
}
...
bool UserPermissionRepository::Delete(int entry) const {
  try {
    ...
    if (!database->RunPrepared("DELETE FROM UserPermissions WHERE Entry = ?",
                               parameters, "UserPermissionRepository::Delete"))
      throw std::runtime_error("[RunPrepared exception]");
  ...
  {
    database->RollbackTransaction();
    throw(std::runtime_error(std::string("[DeleteUserPermission Exception] ") +
                             e.what()));
  }
}
...
int UserPermissionRepository::GetCurrentSequence() const {
  try {
    omnisphere::types::DataTable dataTable = database->FetchResults(
        "SELECT COALESCE(UserPermSequence, 0) + 1 UserPermSequence FROM "
...
bool UserPermissionRepository::UpdateSequence() const {
  try {
    if (!database->RunStatement(
            "UPDATE Sequences SET UserPermSequence = "
            "COALESCE(UserPermSequence, 0) + 1 WHERE Entry = 1"))
