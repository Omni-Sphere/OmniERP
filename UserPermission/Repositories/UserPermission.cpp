...
bool UserPermissionRepository::Create(const omnisphere::dtos::CreateUserPermission& userPermission) const {
    try {
        ...
        return true;
    } catch (const std::exception& e) { database->RollbackTransaction(); throw(std::runtime_error(std::string("[CreateUserPermission Exception] ") + e.what())); }
}
...
bool UserPermissionRepository::Update(const omnisphere::dtos::UpdateUserPermission& userPermission) const {
    try {
        ...
        std::vector<omnisphere::types::SQLParam> parameters;
        if (userPermission.IsAllowed.has_value()) { query += ", IsAllowed = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(userPermission.IsAllowed.value() ? "Y" : "N")); }
