...
omnisphere::models::UserPermission UserPermission::Add(const omnisphere::dtos::CreateUserPermission& userPermission) const {
    if (pImpl->repository->Create(userPermission)) { omnisphere::dtos::GetUserPermission get; get.UserEntry = userPermission.UserEntry; get.PermissionEntry = userPermission.PermissionEntry; return Get(get); }
...
omnisphere::models::UserPermission UserPermission::Modify(const omnisphere::dtos::UpdateUserPermission& userPermission) const {
    if (pImpl->repository->Update(userPermission)) { omnisphere::dtos::GetUserPermission get; get.Entry = userPermission.Entry; return Get(get); }
...
std::vector<omnisphere::models::UserPermission> UserPermission::GetAll() const {
    ...
    for (int i = 0; i < data.RowsCount(); i++) list.push_back({data[i]["Entry"], data[i]["UserEntry"], data[i]["PermissionEntry"], data[i]["IsAllowed"], data[i]["CreatedBy"], data[i]["CreateDate"], data[i]["LastUpdatedBy"].GetOptional<int>(), data[i]["UpdateDate"].GetOptional<std::string>()});
    return list;
}
omnisphere::models::UserPermission UserPermission::Get(const omnisphere::dtos::GetUserPermission& getUserPermission) const {
    ...
    return {data[0]["Entry"], data[0]["UserEntry"], data[0]["PermissionEntry"], data[0]["IsAllowed"], data[0]["CreatedBy"], data[0]["CreateDate"], data[0]["LastUpdatedBy"].GetOptional<int>(), data[0]["UpdateDate"].GetOptional<std::string>()};
}
bool UserPermission::Remove(int entry) const { return pImpl->repository->Delete(entry); }
}
