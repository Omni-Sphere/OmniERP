...
{
    RolePermissionRepository::RolePermissionRepository(std::shared_ptr<omnisphere::data::Database> Database)
        : database(Database) {}
    ...
    bool RolePermissionRepository::Create(const omnisphere::dtos::CreateRolePermission& rolePermission) const
    {
        try
        {
            const std::string query = "INSERT INTO RolePermissions (Entry, RoleEntry, PermissionEntry, IsActive, CreatedBy, CreateDate) VALUES (?, ?, ?, ?, ?, GETDATE())";
    ...
    bool RolePermissionRepository::Update(const omnisphere::dtos::UpdateRolePermission& rolePermission) const
    {
        try
        {
            ...
            { query += ", IsActive = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(rolePermission.IsActive.value() ? "Y" : "N")); }
    ...
    int RolePermissionRepository::GetCurrentSequence() const
    {
        try
        {
            omnisphere::types::DataTable dataTable = database->FetchResults("SELECT COALESCE(RolePermSequence, 0) + 1 RolePermSequence FROM Sequences WHERE Entry = 1");
    ...
    bool RolePermissionRepository::UpdateSequence() const
    {
        try
        {
            if (!database->RunStatement("UPDATE Sequences SET RolePermSequence = COALESCE(RolePermSequence, 0) + 1 WHERE Entry = 1")) throw std::runtime_error("[RunStatement exception]");
