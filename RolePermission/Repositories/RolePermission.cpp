...
namespace omnisphere::repositories
{
    ...
    int RolePermissionRepository::GetCurrentSequence() const
    {
        try
        {
            omnisphere::types::DataTable dataTable = database->FetchResults("SELECT ISNULL(RolePermSequence, 0) + 1 RolePermSequence FROM Sequences WHERE Entry = 1");
    ...
    bool RolePermissionRepository::UpdateSequence() const
    {
        try
        {
            if (!database->RunStatement("UPDATE Sequences SET RolePermSequence = ISNULL(RolePermSequence, 0) + 1 WHERE Entry = 1")) throw std::runtime_error("[RunStatement exception]");
