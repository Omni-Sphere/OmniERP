...
struct RolePermission::Impl {
  std::shared_ptr<omnisphere::repositories::RolePermissionRepository>
      repository;
  explicit Impl(std::shared_ptr<omnisphere::data::Database> database)
      : repository(
            std::make_shared<
                omnisphere::repositories::RolePermissionRepository>(database)) {
  }
};
...
omnisphere::models::RolePermission RolePermission::Add(
    const omnisphere::dtos::CreateRolePermission &rolePermission) const {
  if (pImpl->repository->Create(rolePermission)) {
    omnisphere::dtos::GetRolePermission get;
    get.RoleEntry = rolePermission.RoleEntry;
    get.PermissionEntry = rolePermission.PermissionEntry;
    return Get(get);
  }
  throw std::runtime_error("Error adding role permission");
}
omnisphere::models::RolePermission RolePermission::Modify(
    const omnisphere::dtos::UpdateRolePermission &rolePermission) const {
  if (pImpl->repository->Update(rolePermission)) {
    omnisphere::dtos::GetRolePermission get;
    get.Entry = rolePermission.Entry;
    return Get(get);
  }
...
std::vector<omnisphere::models::RolePermission> RolePermission::GetAll() const {
  ...
  for (int i = 0; i < data.RowsCount(); i++)
    list.push_back(
        ...
        {data[i]["Entry"], data[i]["RoleEntry"], data[i]["PermissionEntry"],
...
omnisphere::models::RolePermission RolePermission::Get(
    const omnisphere::dtos::GetRolePermission &getRolePermission) const {
  omnisphere::types::DataTable data =
      pImpl->repository->Read(getRolePermission);

  if (data.RowsCount() == 0)
    throw std::runtime_error("RolePermission doesn't exist");
  return
