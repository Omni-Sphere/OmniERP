...
bool Customer::Create(const omnisphere::dtos::CreateCustomer &_customer) const {
  try {
    ...
    throw std::runtime_error(std::string("[CreateCustomer Exception] ") +
                             e.what());
  }
}
...
bool Customer::Update(const omnisphere::dtos::UpdateCustomer &_customer) const {
  try {
    ...
    Database->RollbackTransaction();
    throw std::runtime_error(std::string("[UpdateCustomer Exception] ") +
                             e.what());
  }
}
...
omnisphere::types::DataTable Customer::Read(int entry) const {
  try {
    ...
    throw std::runtime_error(std::string("[ReadCustomer Exception] ") +
                             e.what());
  }
}
...
omnisphere::types::DataTable Customer::ReadAll() const {
  try {
    ...
    throw std::runtime_error(std::string("[ReadAllCustomer Exception] ") +
                             e.what());
  }
}
