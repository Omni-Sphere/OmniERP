...
omnisphere::models::Employee Employee::Add(const omnisphere::dtos::CreateEmployee& employee) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[AddEmployee Exception] ") + e.what()); }
}
...
omnisphere::models::Employee Employee::Modify(const omnisphere::dtos::UpdateEmployee& employee) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[ModifyEmployee Exception] ") + e.what()); }
}
...
std::vector<omnisphere::models::Employee> Employee::GetAll() const {
    try {
        ...
        return list;
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetAllEmployees Exception] ") + e.what()); }
}
...
omnisphere::models::Employee Employee::Get(const omnisphere::dtos::GetEmployee& getEmployee) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetEmployee Exception] ") + e.what()); }
}

bool Employee::Remove(int entry) const {
    try { return pImpl->repository->Delete(entry); }
    catch (const std::exception& e) { throw std::runtime_error(std::string("[RemoveEmployee Exception] ") + e.what()); }
}
} // namespace omnisphere::employee
