...
struct Employee::Impl {
  ...
  explicit Impl(std::shared_ptr<omnisphere::data::Database> database)
      : repository(
            std::make_shared<omnisphere::repositories::EmployeeRepository>(
                database)) {}
};
...
omnisphere::models::Employee
Employee::Add(const omnisphere::dtos::CreateEmployee &employee) const {
  try {
  ...
  {
    throw std::runtime_error(std::string("[AddEmployee Exception] ") +
                             e.what());
  }
}

omnisphere::models::Employee
Employee::Modify(const omnisphere::dtos::UpdateEmployee &employee) const {
  try {
  ...
  {
    throw std::runtime_error(std::string("[ModifyEmployee Exception] ") +
                             e.what());
  }
}
...
std::vector<omnisphere::models::Employee> Employee::GetAll() const {
  try {
    ...
    for (int i = 0; i < data.RowsCount(); i++) {
      list.emplace_back(
          data[i]["Entry"], data[i]["Code"], data[i]["Name"],
          data[i]["FirstName"],
          data[i]["SecondName"].GetOptional<std::string>(), data[i]["LastName"],
          data[i]["SecondLastName"].GetOptional<std::string>(),
          data[i]["Phone"].GetOptional<std::string>(),
          data[i]["DateOfBirth"].GetOptional<std::string>(),
          data[i]["PlaceOfBirth"].GetOptional<std::string>(),
          data[i]["Comments"].GetOptional<std::string>(), data[i]["IsActive"],
  ...
  {
    throw std::runtime_error(std::string("[GetAllEmployees Exception] ") +
                             e.what());
  }
}

omnisphere::models::Employee
Employee::Get(const omnisphere::dtos::GetEmployee &getEmployee) const {
  try {
    ...
    return omnisphere::models::Employee(
        data[0]["Entry"], data[0]["Code"], data[0]["Name"],
        data[0]["FirstName"], data[0]["SecondName"].GetOptional<std::string>(),
        data[0]["LastName"],
        data[0]["SecondLastName"].GetOptional<std::string>(),
        data[0]["Phone"].GetOptional<std::string>(),
        data[0]["DateOfBirth"].GetOptional<std::string>(),
        data[0]["PlaceOfBirth"].GetOptional<std::string>(),
        data[0]["Comments"].GetOptional<std::string>(), data[0]["IsActive"],
  ...
  {
    throw std::runtime_error(std::string("[GetEmployee Exception] ") +
                             e.what());
  }
}
...
bool Employee::Remove(int entry) const {
  ...
  {
    throw std::runtime_error(std::string("[RemoveEmployee Exception] ") +
                             e.what());
  }
}
} // namespace omnisphere::employee
