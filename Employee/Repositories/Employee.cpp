...
bool EmployeeRepository::Update(const omnisphere::dtos::UpdateEmployee& employee) const {
    try {
        ...
        if (employee.Code.has_value()) { query += ", Code = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.Code.value())); }
        if (employee.Name.has_value()) { query += ", Name = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.Name.value())); }
        if (employee.FirstName.has_value()) { query += ", FirstName = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.FirstName.value())); }
        if (employee.SecondName.has_value()) { query += ", SecondName = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.SecondName.value())); }
        if (employee.LastName.has_value()) { query += ", LastName = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.LastName.value())); }
        if (employee.SecondLastName.has_value()) { query += ", SecondLastName = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.SecondLastName.value())); }
        if (employee.Phone.has_value()) { query += ", Phone = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.Phone.value())); }
        if (employee.DateOfBirth.has_value()) { query += ", DateOfBirth = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.DateOfBirth.value())); }
        if (employee.PlaceOfBirth.has_value()) { query += ", PlaceOfBirth = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.PlaceOfBirth.value())); }
        if (employee.Comments.has_value()) { query += ", Comments = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.Comments.value())); }
        if (employee.IsActive.has_value()) { query += ", IsActive = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(employee.IsActive.value() ? "Y" : "N")); }
