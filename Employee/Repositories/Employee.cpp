...
bool EmployeeRepository::Create(
  ...
  try {
    ...
    throw(std::runtime_error(std::string("[CreateEmployee Exception] ") +
                             e.what()));
  }
}
...
bool EmployeeRepository::Update(
  ...
  try {
    ...
    database->RollbackTransaction();
    throw(std::runtime_error(std::string("[UpdateEmployee Exception] ") +
                             e.what()));
  }
}
...
omnisphere::types::DataTable EmployeeRepository::ReadAll() const {
  try {
    ...
    throw(std::runtime_error(std::string("[ReadAllEmployee Exception] ") +
                             e.what()));
  }
}
...
omnisphere::types::DataTable EmployeeRepository::Read(
  ...
  try {
    ...
    throw(std::runtime_error(std::string("[ReadEmployee Exception] ") +
                             e.what()));
  }
}
...
bool EmployeeRepository::Delete(int entry) const {
  try {
    ...
    database->RollbackTransaction();
    throw(std::runtime_error(std::string("[DeleteEmployee Exception] ") +
                             e.what()));
  }
}
