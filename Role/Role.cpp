...
omnisphere::models::Role Role::Add(const omnisphere::dtos::CreateRole& role) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[AddRole Exception] ") + e.what()); }
}
...
omnisphere::models::Role Role::Modify(const omnisphere::dtos::UpdateRole& role) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[ModifyRole Exception] ") + e.what()); }
}
...
std::vector<omnisphere::models::Role> Role::GetAll() const {
    try {
        ...
        return list;
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetAllRoles Exception] ") + e.what()); }
}
...
omnisphere::models::Role Role::Get(const omnisphere::dtos::GetRole& getRole) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetRole Exception] ") + e.what()); }
}

bool Role::Remove(int entry) const {
    try { return pImpl->repository->Delete(entry); }
    catch (const std::exception& e) { throw std::runtime_error(std::string("[RemoveRole Exception] ") + e.what()); }
}
} // namespace omnisphere::role
