...
struct Role::Impl {
  ...
  explicit Impl(std::shared_ptr<omnisphere::data::Database> database)
      : repository(std::make_shared<omnisphere::repositories::RoleRepository>(
            database)) {}
};
...
Role::Add(const omnisphere::dtos::CreateRole &role) const {
  try {
...
omnisphere::models::Role
Role::Modify(const omnisphere::dtos::UpdateRole &role) const {
  try {
...
std::vector<omnisphere::models::Role> Role::GetAll() const {
  ...
  {
    throw std::runtime_error(std::string("[GetAllRoles Exception] ") +
                             e.what());
  }
}

omnisphere::models::Role
Role::Get(const omnisphere::dtos::GetRole &getRole) const {
  try {
