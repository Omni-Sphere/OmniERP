...
bool RoleRepository::Create(const omnisphere::dtos::CreateRole &role) const {
  try {
    const std::string query =
        "INSERT INTO Roles (Entry, Code, Name, DepartmentEntry, CreatedBy, "
        "CreateDate) VALUES (?, ?, ?, ?, ?, GETDATE())";
    ...
    throw(
        std::runtime_error(std::string("[CreateRole Exception] ") + e.what()));
  }
}
...
bool RoleRepository::Update(const omnisphere::dtos::UpdateRole &role) const {
  try {
    ...
    if (role.Code.has_value()) {
      query += ", Code = ?";
      parameters.push_back(omnisphere::types::MakeSQLParam(role.Code.value()));
    }

    if (role.Name.has_value()) {
      query += ", Name = ?";
      parameters.push_back(omnisphere::types::MakeSQLParam(role.Name.value()));
    }

    if (role.DepartmentEntry.has_value()) {
      query += ", DepartmentEntry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(role.DepartmentEntry.value()));
    }
    ...
    database->RollbackTransaction();
    throw(
        std::runtime_error(std::string("[UpdateRole Exception] ") + e.what()));
  }
}
...
omnisphere::types::DataTable RoleRepository::ReadAll() const {
  try {
    ...
    throw(
        std::runtime_error(std::string("[ReadAllRole Exception] ") + e.what()));
  }
}

omnisphere::types::DataTable
RoleRepository::Read(const omnisphere::dtos::GetRole &getRole) const {
  try {
    ...
    if (getRole.Entry.has_value()) {
      query += " AND Entry = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getRole.Entry.value()));
    }

    if (getRole.Code.has_value()) {
      query += " AND Code = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getRole.Code.value()));
    }
...
bool RoleRepository::Delete(int entry) const {
  try {
    ...
    database->RollbackTransaction();
    throw(
        std::runtime_error(std::string("[DeleteRole Exception] ") + e.what()));
  }
}

int RoleRepository::GetCurrentSequence() const {
  try {
    const std::string query = "SELECT COALESCE(RoleSequence, 0) + 1 "
                              "RoleSequence FROM Sequences WHERE Entry = 1";
    omnisphere::types::DataTable dataTable =
        database->FetchResults(query, "RoleRepository::GetCurrentSequence");
...
bool RoleRepository::UpdateSequence() const {
  try {
    const std::string query = "UPDATE Sequences SET RoleSequence = "
                              "COALESCE(RoleSequence, 0) + 1 WHERE Entry = 1";
