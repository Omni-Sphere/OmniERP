...
bool RoleRepository::Update(const omnisphere::dtos::UpdateRole& role) const {
    try {
        ...
        if (role.Code.has_value()) { query += ", Code = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(role.Code.value())); }
        if (role.Name.has_value()) { query += ", Name = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(role.Name.value())); }
        if (role.DepartmentEntry.has_value()) { query += ", DepartmentEntry = ?"; parameters.push_back(omnisphere::types::MakeSQLParam(role.DepartmentEntry.value())); }
