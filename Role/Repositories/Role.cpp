...
{
    RoleRepository::RoleRepository(std::shared_ptr<omnisphere::data::Database> Database)
        : database(Database) {}
    ...
    int RoleRepository::GetCurrentSequence() const
    {
        try
        {
            const std::string query = "SELECT COALESCE(RoleSequence, 0) + 1 RoleSequence FROM Sequences WHERE Entry = 1";
    ...
    bool RoleRepository::UpdateSequence() const
    {
        try
        {
            const std::string query = "UPDATE Sequences SET RoleSequence = COALESCE(RoleSequence, 0) + 1 WHERE Entry = 1";
