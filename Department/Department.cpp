...
omnisphere::models::Department Department::Add(const omnisphere::dtos::CreateDepartment& department) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[AddDepartment Exception] ") + e.what()); }
}
...
omnisphere::models::Department Department::Modify(const omnisphere::dtos::UpdateDepartment& department) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[ModifyDepartment Exception] ") + e.what()); }
}
...
std::vector<omnisphere::models::Department> Department::GetAll() const {
    try {
        ...
        return list;
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetAllDepartments Exception] ") + e.what()); }
}
...
omnisphere::models::Department Department::Get(const omnisphere::dtos::GetDepartment& getDepartment) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetDepartment Exception] ") + e.what()); }
}

bool Department::Remove(int entry) const {
    try { return pImpl->repository->Delete(entry); }
    catch (const std::exception& e) { throw std::runtime_error(std::string("[RemoveDepartment Exception] ") + e.what()); }
}
} // namespace omnisphere::department
