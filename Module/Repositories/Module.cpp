...
omnisphere::types::DataTable ModuleRepository::ReadAll() const {
  try {
    ...
    throw(std::runtime_error(std::string("[ReadAllModule Exception] ") +
                             e.what()));
  }
}

omnisphere::types::DataTable
ModuleRepository::Read(const omnisphere::dtos::GetModule &getModule) const {
  try {
    ...
    if (getModule.Entry.has_value()) {
      ...
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getModule.Entry.value()));
    }

    if (getModule.Code.has_value()) {
      query += " AND Code = ?";
      parameters.push_back(
          omnisphere::types::MakeSQLParam(getModule.Code.value()));
    }

    return database->FetchPrepared(query, parameters, "ModuleRepository::Read");
  } catch (const std::exception &e) {
    throw(
        std::runtime_error(std::string("[ReadModule Exception] ") + e.what()));
  }
}
} // namespace omnisphere::repositories
