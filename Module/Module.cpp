...
struct Module::Impl {
  ...
  explicit Impl(std::shared_ptr<omnisphere::data::Database> database)
      : repository(std::make_shared<omnisphere::repositories::ModuleRepository>(
            database)) {}
};

Module::Module(std::shared_ptr<omnisphere::data::Database> database)
...
std::vector<omnisphere::models::Module> Module::GetAll() const {
  ...
  {
    throw std::runtime_error(std::string("[GetAllModules Exception] ") +
                             e.what());
  }
}

omnisphere::models::Module
Module::Get(const omnisphere::dtos::GetModule &getModule) const {
  try {
    omnisphere::types::DataTable data = pImpl->repository->Read(getModule);

    if (data.RowsCount() == 0)
      throw std::runtime_error("Module doesn't exist");

    return omnisphere::models::Module(
        data[0]["Entry"], data[0]["Code"], data[0]["Name"],
        data[0]["DepartmentEntry"].GetOptional<int>(), data[0]["CreatedBy"],
