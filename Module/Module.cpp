...
std::vector<omnisphere::models::Module> Module::GetAll() const {
    try {
        ...
        return list;
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetAllModules Exception] ") + e.what()); }
}
...
omnisphere::models::Module Module::Get(const omnisphere::dtos::GetModule& getModule) const {
    ...
    } catch (const std::exception& e) { throw std::runtime_error(std::string("[GetModule Exception] ") + e.what()); }
}
} // namespace omnisphere::module
