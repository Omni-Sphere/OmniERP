...
namespace omnisphere::models
{
    class Item : public omnisphere::models::BaseModel
    {
        ...
        Item(int _Entry, std::string _Code, std::string _Name,
             std::optional<std::string> _Description, std::optional<std::string> _Image,
             bool _IsActive, bool _PurchaseItem, bool _SellItem, bool _InventoryItem,
             double _Price, std::optional<int> _Brand, std::optional<int> _Group,
             double _OnHand, std::optional<double> _OnOrder,
             std::optional<double> _OnRequest, std::optional<double> _MinStock,
             std::optional<double> _MaxStock, std::optional<double> _MinOrder,
             std::optional<double> _MaxOrder, std::optional<double> _MinRequest,
             std::optional<double> _MaxRequest, int _CreatedBy, std::string _CreateDate,
            ...
            : omnisphere::models::BaseModel(_Entry, std::move(_Code), std::move(_Name),
                                            ...
                                            _LastUpdatedBy, std::move(_LastUpdateDate)),
            Description(std::move(_Description)), Image(std::move(_Image)),
            IsActive(_IsActive), PurchaseItem(_PurchaseItem), SellItem(_SellItem),
            InventoryItem(_InventoryItem), Price(_Price), Brand(_Brand), Group(_Group),
            OnHand(_OnHand), OnOrder(_OnOrder), OnRequest(_OnRequest),
            MinStock(_MinStock), MaxStock(_MaxStock), MinOrder(_MinOrder),
            MaxOrder(_MaxOrder), MinRequest(_MinRequest), MaxRequest(_MaxRequest)
        {
            ValidateItem();
        }
