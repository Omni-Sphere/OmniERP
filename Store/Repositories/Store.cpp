...
static const std::vector<std::string> storeSelectFields = {"Entry",
                                                           "Code",
                                                           "Name",
                                                           "GuestCustomer",
                                                           "Address",
...
Store::Store(std::shared_ptr<omnisphere::data::Database> database)
...
bool Store::Create(const omnisphere::dtos::CreateStore &_store) const {
  try {
    ...
    auto insertData = omnisphere::types::BuildInsertQuery(
        "Stores", GetCurrentSequence(), _store);

    if (!Database->RunPrepared(insertData.Query, insertData.Parameters,
                               "Store::Create"))
    ...
    throw std::runtime_error(std::string("[CreateStore Exception] ") +
                             e.what());
  }
}
...
bool Store::Update(const omnisphere::dtos::UpdateStore &_store) const {
  try {
    ...
    cols.push_back({"LastUpdatedBy",
                    omnisphere::types::MakeSQLParam(_store.LastUpdatedBy)});
    cols.push_back(
        {"UpdateDate", omnisphere::types::MakeSQLParam(_store.UpdateDate)});

    auto updateResult = omnisphere::types::BuildUpdateQuery(
        "Stores", cols, "Entry", omnisphere::types::MakeSQLParam(_store.Entry));

    if (!Database->RunPrepared(updateResult.Query, updateResult.Parameters,
                               "Store::Update"))
    ...
    Database->RollbackTransaction();
    throw std::runtime_error(std::string("[UpdateStore Exception] ") +
                             e.what());
  }
}

omnisphere::types::DataTable
Store::Read(const omnisphere::dtos::GetStore &filter,
  ...
  try {
    ...
    std::string sQuery =
        "SELECT " + qp.SelectClause + " FROM Stores WHERE " + qp.WhereClause;
...
omnisphere::types::DataTable
Store::ReadAll(const std::vector<std::string> &fields) const {
  try {
    const std::vector<std::string> &selectFields =
        fields.empty() ? storeSelectFields : fields;
    ...
    throw std::runtime_error(std::string("[ReadAllStore Exception] ") +
                             e.what());
  }
}

omnisphere::types::DataTable
Store::Search(const std::vector<std::string> &fields,
              const omnisphere::dtos::SearchStore &filter) const {
  try {
    std::vector<omnisphere::types::Condition> conditions;
    ...
    throw std::runtime_error(std::string("[SearchStore Exception] ") +
                             e.what());
  }
}
...
bool Store::UpdateStoreSequence() const {
  try {
    ...
    throw std::runtime_error(std::string("[UpdateStoreSequence Exception] ") +
                             e.what());
  }
}
} // namespace omnisphere::repositories
