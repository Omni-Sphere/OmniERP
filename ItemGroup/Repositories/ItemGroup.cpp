...
static const std::vector<std::string> groupSelectFields = {
    "Entry",      "Code",          "Name",      "CreatedBy",
    "CreateDate", "LastUpdatedBy", "UpdateDate"};

ItemGroup::ItemGroup(std::shared_ptr<omnisphere::data::Database> Database)
...
bool ItemGroup::Create(
  ...
  try {
    ...
    const std::string sQuery =
        omnisphere::types::BuildInsertQuery("Groups", insertColumns);
  ...
  try {
    ...
    if (updateItemGroup.Name.has_value()) {
      ...
      parameters.push_back(
          omnisphere::types::MakeSQLParam(updateItemGroup.Name.value()));
    }

    setColumns.push_back("LastUpdatedBy");
    parameters.push_back(
        omnisphere::types::MakeSQLParam(updateItemGroup.LastUpdatedBy));

    setColumns.push_back("UpdateDate");
    parameters.push_back(
        omnisphere::types::MakeSQLParam(updateItemGroup.UpdateDate));

    const std::string sQuery =
        omnisphere::types::BuildUpdateQuery("Groups", setColumns, "Code = ?");
...
ItemGroup::ReadAll(const std::vector<std::string> &fields) const {
  try {
    const std::vector<std::string> &selectFields =
        fields.empty() ? groupSelectFields : fields;
...
omnisphere::types::DataTable
ItemGroup::Search(const std::vector<std::string> &fields,
  ...
  try {
    ...
    if (itemGroup.Entry.has_value()) {
      ...
      parameters.push_back(
          omnisphere::types::MakeSQLParam(itemGroup.Code.value()));
    } else if (itemGroup.Name.has_value()) {
      conditions.push_back({"", "Name", "LIKE", "?"});
      parameters.push_back(
          omnisphere::types::MakeSQLParam("%" + itemGroup.Name.value() + "%"));
    } else {
    ...
    const std::string sQuery =
        "SELECT " + qp.SelectClause + " FROM Groups WHERE " + qp.WhereClause;
