...
ItemGroup::Add(const std::vector<std::string> &fields,
...
ItemGroup::GetAll(const std::vector<std::string> &fields) const {
  try {
...
omnisphere::models::ItemGroup
ItemGroup::Get(const std::vector<std::string> &fields,
  ...
  try {
    ...
    return omnisphere::data::MapFromRow<omnisphere::models::ItemGroup>(
        dataTable[0]);
  } catch (const std::exception &e) {
