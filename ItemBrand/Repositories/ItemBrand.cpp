...
bool ItemBrand::Create(
  ...
  try {
    ...
    const std::string sQuery =
        omnisphere::types::BuildInsertQuery("Brands", insertColumns);
  ...
  try {
    ...
    if (updateItemBrand.Name.has_value()) {
      ...
      parameters.push_back(
          omnisphere::types::MakeSQLParam(updateItemBrand.Name.value()));
    }

    setColumns.push_back("LastUpdatedBy");
    parameters.push_back(
        omnisphere::types::MakeSQLParam(updateItemBrand.LastUpdatedBy));

    setColumns.push_back("UpdateDate");
    parameters.push_back(
        omnisphere::types::MakeSQLParam(updateItemBrand.UpdateDate));

    const std::string sQuery =
        omnisphere::types::BuildUpdateQuery("Brands", setColumns, "Code = ?");
...
ItemBrand::ReadAll(const std::vector<std::string> &fields) const {
  try {
    const std::vector<std::string> &selectFields =
        fields.empty() ? brandSelectFields : fields;
...
omnisphere::types::DataTable
ItemBrand::Search(const std::vector<std::string> &fields,
  ...
  try {
    ...
    const std::string sQuery =
        "SELECT " + qp.SelectClause + " FROM Brands" +
